...
impl Game {
    ...
    fn calculate_set_power(&self) -> i32 {
        return self.highest_red * self.highest_blue * self.highest_green;
    }
}
...
fn parse_line_to_game_data(line: &str) -> Game {
    ...
    // Find the minimum possible number of each color for each game, multiply the colors against each other, then sum the numbers from each game together
...
fn part2(input: &str) -> i32 {
    ...
    return lines
        ...
        //.map(|game| game.id)
        .map(|game| game.calculate_set_power())
        .sum::<i32>();
}
